...
my $h = substr($line,4,index($line, "h")-4);
print STDOUT "$h\n";
system("echo $h > coord_RA.txt");

my $m = substr($line,index($line,"h")+1,index($line, "m")-index($line,"h")-1);
print STDOUT "$m\n";
system("echo $m >> coord_RA.txt");

my $s = substr($line,index($line,"m")+1,index($line, "s")-index($line,"m")-1);
print STDOUT "$s\n";
system("echo $s >> coord_RA.txt");
